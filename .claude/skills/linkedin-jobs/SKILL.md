...
## 지역은 확인 항목이지 1차 필터가 아니다

회사와 직무를 먼저 보고, 근무지는 낼 때 확인한다. **`-l/--location` 으로 세부 지역을 좁히지
마라.** LinkedIn 서버 필터는 광역으로 번진다. 실측:
...
기본은 `-l "South Korea"` 로 넓게 긁고 지역으로 자르지 않는 것이다. GLG가 특정 지역만
별도로 보라고 명시했을 때만 `--loc` 로 소프트 필터한다:
...
긁고 자른다). 6건 얻는 데 30건 본문 요청 ≈ 40초. 명시적인 지역별 판을 만들 때 치를 값이다.
...
근무지를 **둘째 줄에 못 박는다.** 1차 필터는 아니지만, 낼 때 확인해야 하는 조건을 목록에서
숨기면 브라우저 축이 미확인인 줄 모르고 지원하게 된다.
...
- 공고 본문을 요약해서 GLG 에게 올릴 때 **연차·비자·근무지는 원문 표현 그대로** 옮긴다.
  여기서 의역하면 잘못 지원한다.
- **모르는 근무지를 채우지 않는다.** `?` 는 `?` 로 올린다. 여기서 추정하면 제출 단계가
  확인된 사실과 추론을 구분할 수 없게 된다.
