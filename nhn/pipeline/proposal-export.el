...
;; Author: Junghan Kim <[email removed]>
...
(defvar doom-emacs-dir
  (or (getenv "EMACSDIR")
      ;; `expand-file-name' returns a string for a path that does not exist, so the old
      ;; `(or … (expand-file-name "~/doomemacs-starter") …)' chain always latched onto the
      ;; first candidate even when its directory was absent — and crashed later trying to
      ;; load a core that was not there. Probe for the straight build instead and take the
      ;; first core that actually carries one.
      (let ((found nil))
        (dolist (d '("~/.emacs.d" "~/doomemacs" "~/.config/emacs" "~/doomemacs-starter"))
          (let ((dd (expand-file-name d)))
            (when (and (not found)
                       (file-directory-p (expand-file-name ".local/straight" dd)))
              (setq found dd))))
        found)
      (expand-file-name "~/.config/emacs")))

;; Packages are loaded from the straight build directory directly (see below), not through
;; Doom's own bootstrap — so this only needs `user-emacs-directory' set to point the straight
;; probe at the right core. The old code hard-loaded lisp/doom.el + lisp/doom-start.el, but
;; Doom 30.2 renamed doom-start.el to doom-emacs.el and some cores ship neither; under
;; `debug-on-error' a missing file aborted the whole batch. Load nothing here that can fail.
