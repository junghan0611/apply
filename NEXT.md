# NOW — ✅ **2026-08-01 ax 증거면을 생성 루틴에 박았다.** 다음은 **업스테이지 제출(A)**

- **Current**: `ax.junghanacs.com`(공개 포트폴리오·에이전트 철학 페이지)이 **생성 표면 어디에도
  없었다** — 이력서 6컷·깊이 문서 3종 전부 0건. 07-29 에 「별도 트랙」으로 가른 판단이 맞았는데
  **링크를 거는 일까지 함께 갈라진 것**이 원인이다. 이제 연락처 줄 5자리 + `references.bib`
  `@live-ax` + `competency` §근거 · `portfolio` §검증 · `detail` §읽는 법에 실린다.
  **`run.sh` verify 가 산출물 PDF 텍스트에서 직접 검사**하고, 빼고 돌려서 **6컷 전부 실패하는 것을
  실측**했다. 활성 세트 **10건 재생성 완료**, `check.py` · `--deep` **33건 통과**.
- **Next**: **A — 업스테이지 `AI Engineer - Agents` 제출.** 관문(문항 1 제품 체험 평가)은 07-30 에
  닫혔고 세트 3종은 `submit/`에 깔려 있다(셋 다 ax 실림 확인). 남은 건 **GLG 손** —
  경력사항 3건의 ⚠칸(부서·직급·정확한 월)과 첨부·법적값. 폼: `careers.upstage.ai/ko/o/194880`.
  ⚠ 제출 직전 `applications/alive.py` 를 당일 판으로 다시 돌린다.
- **Read**: `applications/upstage--ai-engineer-agents/submission.md` §폼 전체 구조 실측(07-31 재확인)
  + `answers.md` §0(경력사항 ⚠칸이 어디인지).
- **Blocker**: 없음. ⚠ 원장 Upstage 행의 지원일 셀에 `(2026-07-31 목표 제출)`이 남아 있다 —
  제출하면서 실제 지원일로 덮는다.
- **🔴 이 건이 드러낸 것**: **트랙을 가르는 것과 가리키지 않는 것은 다른 일이다.** 분리 판단이
  옳아도 *참조까지 끊겼는지* 는 따로 봐야 한다. 게이트가 없던 자리라 **아무도 거짓말하지
  않았는데 9개 문서가 조용히 비어 있었다** — 원장 §계보에 박았다.
- **🔴 이 건이 드러낸 것 두 가지 — 원장 §계보에 박았다** (2026-08-01 GLG: *"시간 축으로 이
  작업의 타임라인이 이 작업 자체를 강화시키고 있다. 그래서 원장에 잘 기록해야 된다."*)
  1. **계약은 새 건에 자동으로 붙지 않는다.** 07-30 에 고친 「이력서 한 장」이 07-31 에 만든
     이 건에서 **그대로 재발**했다 — `submission.md` 에 `추가 첨부` 행이 없으면 `stage.py` 는
     dossier 를 깔 **경로 자체가 없다.** → `AGENTS.md` §건 하나 내는 순서에 **3b** 를 넣었다.
  2. **게이트가 조용히 거짓말하고 있었다.** 원장 상태 셀을 `**ready**(…목표 제출)` 처럼 꾸미면
     `check.py` 가 그 행을 **통째로 건너뛰면서 `ok`** 를 냈다 — **4건이 그렇게 빠져 있었다**
     (AutoEver · Upstage · XCENA · IGNITE). fail-closed 가드를 넣어 **검사 대상 30 → 33건**.
     `aim_leak` fail-open(07-30 GPT 재검 P0-C)과 **같은 종류**다. 상태 셀에는 **순수 상태어
     하나만** 두고 부연은 다른 열로 옮긴다.

---

## ✅ 2026-08-01 현대오토에버 서류합격 후 상세정보 기재 제출 완료 — 다음 통보 대기

- **🎉 현대오토에버 `[Tech] AI Agent 엔지니어` 첫 서류합격 회신**(`[email removed]`,
  도메인 `greetinghr.com`) 뒤의 상세 정보 기재를 **2026-08-01 GLG가 최종 제출**했다. 이력·학력·
  병역·사전 질문 3건과 이력서·자기소개서 PDF 2종을 냈고, 완료 화면 원문은
  `applications/hyundai-autoever--ai-agent-engineer/submit/KimJunghan_AutoEver_Detail_Form.md`에 있다.
  원장 상태는 `replied` 유지 — 다음 전형 **ACT(인성검사) + 코딩테스트**의 실제 응시 창 통보를 기다린다.
  - ✅ **ACT·코딩테스트 실측 조사 완료(2026-07-31)** — 코딩테스트는 **프로그래머스, 3시간·
    2문항, 그래프 알고리즘 위주 고난도**(백준 골드상위~플래 체감), **프록터링 엄격**(모니토
    앱·웹캠·화면공유·폰 측후면 촬영·필기 볼펜+종이 1장). ACT 는 HMAT 계열 컬처핏 검사,
    ~1시간, 회사가 응시 창을 지정 통보. 신입/타 직군 후기 기반이라 **이 건(경력)과 다를
    수 있다** — 근거·출처는 `submission.md` §다음 전형 대비.
  - ✅ **상세정보 기재 완료(2026-08-01)** — 8/3 하드 데드라인 전에 닫았다. ACT·코딩테스트 실제
    응시일은 아직 회사 통보 전이며, 잠정 준비 블록은 확정 일정이 아니다.
  - ⚠ **감독형 시험 경계** — 코딩테스트는 웹캠·화면공유가 붙는 프록터링 시험이다.
    **응시 중 에이전트 보조는 하지 않는다**(`AGENTS.md` §경계 새 항목).
  - 🔴 **2026-07-31 GLG 방침 — 대비 학습을 하지 않는다.** *"전통방식을 위해서 내가 공부할
    일은 없어. 그렇게 시간 쓰고 싶지 않거든. 상황을 보면서 진행하면 돼. 경력은 조금
    다를 수도 있을 거야. 지금 코딩테스트 사이트가 사라지는 중이라 이거 어떻게 바뀔지
    몰라."* 위 조사 내용은 잠정 참고용이지 학습 계획이 아니다. 에이전트는 연습 문제 세트나
    준비 일정을 미리 만들지 않는다 — 실제 통보가 오면 그때 대응한다. `FAQ.md` §11 이 정본.
  - **이 건이 「서류합격 → 후속전형」 루프의 첫 사례다.** 처리 순서를 `AGENTS.md` §제출
    이후에 여섯 단계로 박아 두었다 — 다음 회사도 같은 순서로 처리한다.

> 🔢 **카페 세션 잔여 실행(2026-07-30 기준)도 아직 안 끝났다** — `LEDGER.md` 의 `ready` 행 수
> **빼기 보류 2**(Cohere · OpenAI). 11:5x 기준 `ready` 7 − 보류 2 = **실행 5**.

- **Hot group**: 지원 실행. 공개 준비는 이 축이 아니다.
- **▶ 다음 한 걸음 — 남은 5건 중 넷은 폼만 열면 끝난다. 글이 필요한 건은 하나도 없다.**
  1. **Telechips** `careers.telechips.com/job_posting/iSXmKnRT/apply` — Embedded 2쪽.
     ⛔ **ninehire 라 GLG 가 직접 올린다**(아래 §ninehire 계약)
  2. **Sonatus** LinkedIn 간편지원 `/jobs/view/4380679251/` — ⚠ **저장된 이력서가 기본
     선택**이라 파일을 갈아준다. 로그인 축이라 GLG 손
  3. **Bear Robotics**(Breezy) · **Telit**(Paylocity) — **에이전트가 폼을 채울 수 있다.**
     Bear 는 커버레터가 **텍스트 칸**이고 Telit 은 ⚠ `Country` 기본값이 `United States` 다
  4. **MakinaRocks**(그리팅) — FDE 컷. 폼 미실측
- **🔴 글이 필요한 건은 카카오뱅크 DW 하나다** — 자기소개서 2문항 × 1,000자.
  옆 레인(오푸스2)이 `pre-question.md` 까지 세웠고 **GLG 판단 2건이 글을 막고 있다**:
  ① **서버 개발자 - 뱅킹**(`260505`, D-11)도 낼지 — JD 는 가장 정면인데 **Kotlin/Spring 필수
  미충족** ② **이력서 컷** — 데이터 축 컷이 없다. `AI Engineer` 그대로 vs `Data Engineer` 신규.
  **②가 정해지면 문항 2 → 문항 1 순서로 쓴다.**
  - 📌 **카카오 리퍼러는 선배 추천을 받으려고 만들어 둔 것이다** (2026-07-30 GLG).
    지원경로/추천인 칸을 해석할 때 이 사실을 기준으로 본다.
- **🆕 2026-07-30 11:2x — 카카오뱅크 「공고」 축이 새로 열렸다. 인재풀과 별건이다.**
  GLG: *"인재풀만 등록한 거야. 그건 직무 채용보드에 제출한 게 아니라."* →
  케이스 **`applications/kakao-bank--data-engineer-data-warehouse`** · **`draft`** ·
  **자기소개서 2문항 × 1,000자가 관문**이다. GLG: *"지금 못내겠다. 이걸 적어야 한다."*
  - 🔴 **채용홈이 개편됐다** — `kakaobank.recruiter.co.kr/appsite` 는 안내만 띄우고
    목록은 **`recruit.kakaobank.com`**(Next.js) 에 있다. 지원 폼만 recruiter.co.kr 이다.
    경로는 `__BUILD_MANIFEST.sortedPages` → `/jobs`. `boards.py` 로는 안 훑힌다.
  - **열린 25건 중 개발 축 8건 본문을 떴다. `AI` 직군 0건**(마감된 「AI 검색 엔지니어」가
    관련 공고에 흔적으로 남아 있다 — **다시 열리면 이 회사에서 가장 정면인 자리**).
  - ⭐ **DW 를 골랐다** — 우대에 **「에이전틱 코딩(Agentic Coding) 경험」** 이 박혀 있다.
    🔴 하드 갭은 필수 항목 하나, **DW 구축·운영**. 자기소개서에서 숨기지 않는다(Bear 화법).
  - ⛔ **GLG 판단 대기 2건**: ① **서버 개발자 - 뱅킹**(`260505`, D-11)도 낼지 — JD 서사가
    이 회사에서 가장 정면(*Claude Code·Copilot Enterprise 팀 전체 실무 사용* · AI 도구
    프로세스 통합이 담당업무)인데 필수가 **Kotlin/Java + Spring Boot 실무 1년+** 이고
    **이력서에 JVM 0건**이다. ② **이력서 컷** — 데이터 축 컷이 없다. AI Engineer 그대로 vs
    **`Data Engineer` 컷 신규**(`resume/AGENTS.md` §새 타깃 컷).
  - 🔴 **GLG 만 채우는 5칸** — 생년월일·성별·병역·**현주소(신규)**·**고등학교**.
    인재풀 건과 같은 벽이고 하나 늘었다. 동의 2건(개인정보·민감정보)은 **필수 · 거부 시 지원 불가**.
  - ✅ 이 폼은 **연봉·입사 가능일을 묻지 않는다.** `check.py` **30건 일치**.
- **🆕 2026-07-30 10:2x — 별동대 1건 합류: 카카오헬스케어 `[경력] AI Agent Engineer`.**
  GLG 가 URL 을 직접 건넸다(`recruit.kakaohealthcare.com/job_posting/e9Q82rF9`).
  케이스 `applications/kakao-healthcare--ai-agent-engineer` · **`ready`** ·
  세트 4종(`_AI` 이력서 + Competency 5쪽 + Portfolio 12쪽 + 커버레터 1,325자).
  `check.py --deep` **28건 통과** · `alive.py` **열림 확인**(`in_progress`·`until_filled`).
  - ✅ **GPT 교차검수 통과 — P0 0건.** GPT 가 게이트 3종을 독립 재실행해 같은 결과를 얻었다.
    반영 1건: **커버레터 1,000자 축약판(874자)** 을 `cover-letter.md` §축약판에 뒀다.
    상한이 1,000자면 문단을 덜어내지 말고 **그 절을 통째로** 쓴다 — 덜어내면 **경계 문단과
    증거 좌표(URL)가 먼저 잘린다.** `stage.py` 는 축약판을 뽑지 않는다(정본은 1,325자판).
  - ⭐ **JD 가 이 배치에서 가장 정면이다** — *"기성 프레임워크의 조립이 아니라 밑바닥부터"*
    harness · **subagent/skill/workflow 인터페이스** · 평가·관측 · **온프레/클라우드 LLM 라우팅** ·
    **STT/OCR 도입 판단**. 담당 업무 ①②③이 `entwurf`·`agent-config`·`andenken` 에 1:1 로 붙는다.
  - 🔴 **폼 미실측**(ninehire 화이트라벨 · 클라이언트 렌더). **회사 개인정보 고지 기준
    「현재 연봉(고정급)」이 필수 항목**이다 — 이 배치에서 처음이고 **GLG 가 직접 입력**한다.
    생년월일·성별·주소(읍면동)도 필수.
  - 🔴 **`INBOX.md` 의 2026-07-29 「카카오헬스케어 두 건, 둘 다 못 낸다」를 정정했다.**
    판정 자체는 그 두 건에 대해 사실이었고, **세 번째 공고를 못 본 것이 발견 경로의 구멍**이다 —
    자체 보드는 목록이 클라이언트 렌더라 `boards.py` 로 안 훑히고 `careers.kakao.com`
    공동체 목록에도 이 건이 안 뜬다. **한 회사를 「없음」으로 닫기 전에 자체 채용홈을 사람 눈으로 연다.**
  - ⚠ **1차 면접에 라이브 코딩테스트**가 있다 — 한화 건 기조와 부딪칠 수 있는 자리다. **GLG 판단.**
...
## ⭐ 2026-07-30 카페 세션에서 확정된 계약 — 다음 세션은 이것부터 읽는다

**12건을 실제로 내면서 폼을 실측했다. 아래는 추론이 아니라 관찰이다.**

### ① 첨부는 「칸이 있는가」 다음에 「독자가 읽을 언어인가」를 본다

같은 「파일 슬롯 하나」인데 판정이 셋으로 갈렸다. 기준은 슬롯 개수가 **아니다.**

| 건 | 판정 | 근거 |
|---|---|---|
| 카카오헬스케어 | **20쪽 합본** (이력서3+역량5+포트12) | 고지가 필수 항목을 「이력서, **경력기술서** 등」으로 명시 |
| 42dot · Toss | **8쪽 합본** (이력서3+역량5) | `Resume` 칸이 국문 경력기술서를 받을 유일한 자리 · 국문 조직 |
| **Reflection AI** | **이력서 3쪽만** | **폼이 영문이고 깊이 문서는 국문** — 붙여도 읽히지 않는다 |
| Lunit · SOCAR · FuriosaAI | 이력서 + **Portfolio 12쪽** | 선택 슬롯이 **하나**뿐 → 계약대로 Portfolio |
| FriendliAI | 이력서만 | 슬롯 1개 · 서술형·선택첨부 **둘 다 없음** |

- **합본 명령은 `gs` 한 줄이고 선례가 셋이다** — 각 건 `submission.md` §합본 재현.
  `-dPDFSETTINGS=/prepress -dDetectDuplicateImages=true`. **한글·인포그래픽 무손상 확인됨.**
- **합본은 `build/` 에 둔다.** `submit/` 은 `stage.py` 가 MANIFEST 를 다시 쓰는 자리다.
  `check.py` 가 이미 정했다 — *「PDF 는 git 밖이다. 지문과 재현 경로가 증언을 대신한다」*.
- 📌 **다음 판 후보: 깊이 문서 영문판.** 외국계 5건(Reflection·Cohere·OpenAI·Sonatus·Telit)에서
  반복해 걸린다. Embedded 조준판도 아직 없다.

### ② ninehire 는 에이전트가 파일을 못 올린다 — 이제 계약이다

**파일 버튼 뒤에 `input[type=file]` 이 DOM 에 없다**(hidden file input 조회 0건). 네이티브
다이얼로그를 여는 구조라 자동화가 닿지 않는다. **Enhans → 한화비전으로 두 번 확인**했다.
→ **ninehire 계열은 처음부터 GLG 업로드로 계획한다**(한화비전 · 카카오헬스케어 · **Telechips**).

### ③ 서술형은 칸이 묻는 것만 답한다 — 경계 문단의 자리 (`applications/FAQ.md` 정본)

**GLG 가 같은 날 두 번 같은 방향으로 깎았다.** 규칙은 `FAQ.md` §칸이 묻는 것만 답한다.

- **능력을 묻는 칸**(자유 서술 · Career Summary) → 경계 문단 **넣는다.** 빼면 부풀린 글이 된다
- **동기를 묻는 칸**(`Why are you interested…` · 문의 사항) → **넣지 않는다.**
  묻지 않은 답이 마지막 인상 자리를 차지한다. 경계는 `submission.md` 에 두고 면접에서 답한다
- **Lunit 문의칸 2문장**(관찰 → 질문 → 기여)이 그 정본이고 **GLG 스타일**로 `FAQ.md` §9 에 올렸다.
  ⭐ **「운영 표면」 같은 GLG 어휘는 눕히지 않는다** — 남의 말로 쓴 문장이 아니라는 게 드러난다

### ④ 회사 축이 이력을 쓰는 방식을 바꾼다 (FuriosaAI 교훈)

에이전트 초안은 **임베디드 이력을 일부러 뺐고 그 판단이 틀렸다.** GLG: *"여기 사실 임베디드
AI 칩 관련 회사잖아. 시스템소프트웨어로 시작한 게 나의 이력이니까."*
**도메인을 아는 척하는 것과 그 도메인을 지나온 이력을 대는 것은 다른 일이다.**

- **「얇게 검증면을 잡아간다」의 근거가 이력서 안에 있었다** — `resume/body.org:44-51`
  (`single HubState` · `pure transition()` → driver·protocol·cloud 가 같은 검증면, 펌웨어 무수정
  두 네트워크). 이것이 `entwurf`·`andenken` 과 **같은 방법**이고 그 연속성이 글의 척추가 된다
- **사용자가 누구인지로 자리를 읽는다** — FuriosaAI 사내 에이전트의 사용자는 칩·컴파일러·커널
  엔지니어다. Lunit 이 「500명 사내 인프라」였던 것과 같은 구조다

## ▶ 지금 여기서 이어받는다 — 서버가 받았다 (2026-07-29 19:20 오라클)

**노트북 판을 서버가 `git pull` 로 받아 대조했다**(18:36 커밋 두 개까지 들어옴).
**GLG 는 회사라 제출하지 못했다 — 오늘 밤 집에서 낸다.** 아래 네 폴더는 실물 확인됐다.

**⭐ 서버 레인은 「문서」다 (2026-07-29 GLG)** — **PDF 생성은 신경 쓰지 않는다.**
여기서는 md/org 문서 작업만 한다. 빌드·합본·변환은 집(노트북) 축이다.

**분업 — 에이전트가 쓰고, 다음 판에서 GPT 가 교열한다 (2026-07-29 GLG)**
아직 안 낸 제출처의 문서 작업은 이쪽이 초안·구조·사실 확인까지 밀고,
문장 교열과 교차검수는 GPT 가 받는다. **같은 파일을 동시에 잡지 않는다.**

**서버가 팔 문서 — 실행 15건 중 글이 남은 곳** (폼 실측은 브라우저 축이라 여기서 못 한다)

- ~~**AIRS Medical**~~ — ✅ **국문 커버레터 초안 완료** (`cover-letter.md`, 1,710자).
  Holiday 판 골격(§붙여넣을 본문 + §왜 이렇게 썼는가 + §쓰지 않은 것)을 따랐고
  `submit/cover-letter.txt` 로 깔려 있다. ⚠ **그리팅 폼에 칸이 있는지는 미실측** —
...
아니다. **pi 에 Upstage API 를 붙이고 · Document Parse 를 pi 스킬로 만들고 · entwurf 로
돌려 보는 일**이고, 그 결과가 곧 답이다. **GLG 가 실제로 갈 방향과 같다.**
정본은 `applications/upstage--ai-engineer-agents/submission.md` §관문.
⚠ 미확인 하나 — **업스테이지의 「바이브코딩」 이야기**(링크드인에서 봄).
**원문을 찾기 전엔 쓰지 않는다.**

**서버가 대조하며 잡은 것**

- ✅ `check.py` **27건 일치** · 한화 포트폴리오 **18쪽** · 이력서 컷 3쪽 · 표지 1쪽
- ✅ **자가측정 수치 잔재 0건** — 검수 4건 PDF 텍스트 기준
- ⚠ **고쳤다**: `upstage/submission.md` §관문 집필 골격 5번이 **`pgvector 2,945 임베딩` 을
  쓰라고 지시**하고 있었다. 걷어낸 숫자다 — **집필 가이드가 옛 수치를 되살리는 자리였다.**
- 🔴 **고쳤다 — 15건이 아직 `⚠ GLG 확정 필요` 를 들고 있었다.** `FAQ.md` §4 는 10:40 에
  닫혔는데 **각 건 §폼에 답한 질문 표가 따라오지 않았다.** 「이제 폼 앞에서 멈추지 않는다」는
  이 문서의 주장과 실물이 갈라져 있었고, **오늘 밤 폼 앞에서 그 ⚠ 를 봤을 것**이다.
  희망 연봉·입사 가능일·이직 사유 **45행을 한 번에 동기화**했다.
  제출 완료된 현대오토에버는 제외 — 그때 나간 기록이 사실이다.
  **남은 ⚠ 는 정당한 둘뿐**: Cohere 20–40% · OpenAI APAC 30% **출장 비율**.
- ⚠ `alive.py` 는 **06:36 판**이다. **집에서 제출 직전에 다시 돈다.**
- 📝 이 서버엔 `pdftotext` 가 없다. PDF 본문을 뒤질 일이 생기면 `pdftotext` 는
  **빈 결과를 내고 그게 「깨끗함」으로 읽힌다.** `gs -sDEVICE=txtwrite` 를 쓴다.
...
- [2026-07-30] ⭐ **하루 12건 제출 · 누적 17건.** Holiday · AIRS · Lunit · SOCAR · FriendliAI ·
  카카오헬스케어 · FuriosaAI · 42dot · Reflection · 카카오뱅크(인재풀) · 한화비전 · Toss.
  아침에 `dossier/` 를 **회사 중립판으로 재빌드**해 깊이 문서 2종이 세트에 깔리기 시작한 날이고
  (`stage.py` 가 `추가 첨부` 행을 읽게 확장), **폼 실측으로 첨부 판정 기준이 셋으로 갈렸다**
  (위 §카페 세션에서 확정된 계약). `check.py --deep` **30건 일치 · 깊이 29개 조준 통과**.
