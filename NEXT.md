# NOW — ⭐ **2026-07-31 현대오토에버 서류전형 합격.** 상세정보 기재 **D-3(8/3 마감)**

- **🎉 2026-07-31 08:2x — 현대오토에버 `[Tech] AI Agent 엔지니어` 서류전형 합격.** 이 저장소
  전체에서 **첫 서류합격 회신**이다(`[email removed]`, 도메인 `greetinghr.com`).
  다음 전형은 **온라인 인성검사(ACT, Autoever Culture-fit Test) + 코딩테스트**이고, 그 전에
  **상세 정보 기재가 관문** — `https://career.hyundai-autoever.com/o/210271`,
  **2026-08-03 까지**, 「약식 공고 지원정보와 동일하게 입력」, **미기재 시 불합격 처리**.
  - 🔴 **GLG 손이 필요하다** — 생년월일·성별·병역·주소 등 개인정보 칸은 `FAQ.md` §5 원칙대로
    **GLG 가 브라우저에서 직접 입력**하고 Git 에는 값을 남기지 않는다. 로그인 축이라 대신
    채울 수 없다.
  - 기록: `applications/hyundai-autoever--ai-agent-engineer/submission.md`
    (§상태·§이후 기록·§다음 전형 대비). `LEDGER.md` 상태 `submitted` → `replied` 로 갱신함.
  ...
  - ✅ **개인 구글 캘린더 반영(2026-07-31)** — `2026-08-03 09:00` 하드 데드라인(상세정보
    기재) + `2026-08-01~02` 주말 잠정 준비 블록(상세정보+ACT+코테). **ACT·코테 실제
    응시일은 회사 통보 전**이라 잠정 블록이지 확정 일정이 아니다.
