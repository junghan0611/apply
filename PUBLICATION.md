...
### commit ident — 공개 전환 이후의 판정 (2026-09-04 GLG)

`--repo` 게이트의 `commit ident 전수 noreply`는 **정제 시점의 계약**이다. 공개 전환(2026-08-27)
이후에 쌓인 커밋은 그 rewrite를 거치지 않으므로, 로컬 git 설정이 개인 이메일이던 커밋이 하나라도
섞이면 게이트가 계속 FAIL한다. 실제로 `33d0808`·`3cc1b10`(2026-08-28)이 그 상태다.

**GLG 판정: 개인 이메일은 상관없다. 이 실패로 작업을 멈추지 않는다.** history 재작성과 force
push는 하지 않는다 — 이미 공개된 값이고, 되돌려도 회수가 아니다.

그러니 이 항목은 **다시 올리지 않는다.** 게이트 출력에 이 줄이 보이면 여기를 가리키고 지나간다.
남은 이덴트 실패가 *다른* 값(개인 이메일이 아닌 무엇)이면 그때는 새 판정 대상이다.
