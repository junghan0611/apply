...
| 항목 | 값 |
|---|---|
| 상태 | **ready** — ⭐ **바로 낼 수 있다.** 경력기술서 판단은 **A(이력서만)** 로 닫혔다 (2026-07-29) |
| 지원일 | — |
...
**A(이력서만 낸다)로 닫혔다.** 폼의 필수 조건을 이력서 하나로 전부 충족하고,
**새 문서 작업으로 제출을 지연시키지 않는다.** 근거는 아래 §경력기술서 판단.
...
4. **Resume(필수)** — `resume/build/KimJunghan_Resume_AI_Engineer.pdf` **(3쪽)**
   ...
   - 「추가 제출 서류(포트폴리오 등)」(선택)는 **비워 둔다**
   - 「본 포지션에 대한 문의 사항」(선택)도 **비워 둔다**
5. YES/NO 셋: 연봉정보 삭제 확인 **YES** · 개인정보 동의 **YES** · **민감정보 동의 YES**
   (추가 서류를 안 내므로 「추가 지원 서류 수집 동의」는 체크 불필요)
