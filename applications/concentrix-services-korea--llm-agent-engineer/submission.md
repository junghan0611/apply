...
| 지원일 | — |
| 경로 | **원티드 자체 지원** (`wd/379871`) — `out_link: null` 실측 (2026-09-18) |
...
| 항목 | 값 | 영수증 |
|---|---|---|
| 공고 상태 | `active` — 살아 있다 | `GET /api/chaos/jobs/v1/379871/details` (2026-09-18 실측) |
| 마감일 | 없음(`due_time: null`) — 상시라는 보장이 아니라 **예고 없이 닫힌다**는 뜻 | 〃 |
| 경력 요건 | 4~8년 | 〃 `annual_from` / `annual_to` |
| **지원 경로** | **원티드 자체 지원** — 회사 자체 ATS 로 튕기지 않는다 | 〃 `out_link: null`. 대조군 확인은 `WANTED-APPLY-FORM.md` §지원 경로 |
| 추가 지원 유형 | 외국인·장애인 지원 가능 (`['foreigner', 'disabled_person']`) | 〃 `additional_apply_type` |
