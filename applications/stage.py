...
SLOT = {
	...
	# 임베디드 축의 깊이 문서. AX 축 두 문서와 다른 자리다 — 폼이 포트폴리오를 **필수**로
	# 받는데 AX 판이 답이 아닐 때 그 칸으로 간다(텔레칩스). 선택 슬롯만 있는 건에서는
	# 「추가 첨부」로 간다(Bear Robotics `기타 첨부 서류`, Telit `Upload Additional Files`).
	# 이 줄이 없으면 README 의 칸이 `—` 로 비고, 그러면 「이 폴더가 올라갈 파일 전부다」가
	# 어느 칸인지 말하지 못한다 — 08-01 IGNITE 에서 닫은 계약이 다시 열린다.
	"_Experience_Detail": "포트폴리오 / 추가 첨부 (3쪽) — **임베디드 축** 깊이 문서. ⚠ AX 축 문서(`_Competency`·`_Portfolio`)와 섞지 않는다",
	# 증명사진. **git 에 없다** — `.gitignore` 가 `submit/*.jpg|png` 를 막는다. 얼굴 이미지는
	# 히스토리에 넣지 않고 지문과 재현 경로만 남긴다(PDF 와 같은 계약). 그래서 다른 기계에서는
	# 이 파일이 없고, 그 건 `submission.md` 의 크롭 명령으로 다시 만든다.
	"_ID_Photo": "증명사진 — 폼의 「증명사진」 칸. ⚠ **git 에 없다**(얼굴 이미지 비커밋 계약). 원본·크롭 명령은 그 건 `submission.md`",
