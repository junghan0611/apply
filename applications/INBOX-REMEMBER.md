...
판정 기준은 `AGENTS.md` §시스템 축(2026-09-18 GLG). 로그인된 브라우저로 정면 3건 + 본문 미확인
7건을 다시 열어 담당업무 원문을 인용해 판정했다. 리멤버는 검색어가 회사명 토큰을 그대로 태우지
않을 때가 있어(예: "인포시즈"는 정상, "인핸스"는 "인헨스"로 쳐야 걸림) 카드 목록을 스크린샷으로
확인한 뒤 열었다. **URL·엔드포인트 패턴은 여기 적지 않는다** — 회사·직무·리멤버 id·판정·인용만
남긴다. 어느 화면에서도 「지원하기」·「간편 지원하기」는 클릭하지 않았다.

### 정면 3건 — 생존 여부부터

| 회사 | 직무 | 리멤버 id | 생존 |
|---|---|---|---|
| 파트리지시스템즈 | Forward Deployed Engineer | 331625 | 열려 있음(마감일 표시 없음) |
| 스켈터랩스 | Forward Deployment Engineer(FDE/AI Engineer) | 332849 | 열려 있음(마감일 표시 없음) |
| 모레 | AI Agent System Engineer | 329027 | **마감된 공고**(페이지 하단에 "마감된 공고" 표시) — 9일 사이 닫혔다 |

### ✅ 시스템 축 — 낸다

| 회사 | 직무 | 리멤버 id | 담당업무 인용 (판정 근거) |
|---|---|---|---|
| 모레 | AI Agent System Engineer | 329027 | 「멀티 에이전트 기반 인지·추론·계획·실행 파이프라인의 아키텍처 설계 및 구현... 에이전트 간 통신과 외부 도구·데이터·실행 시스템 연동을 위한 인터페이스 및 오케스트레이션 구현... 실행 과정의 상태, 근거, 모델 출력, 도구 호출 및 결과를 추적할 수 있는 설명·감사·재현 체계 구축」— **가장 정면.** 단, 위 표대로 이미 마감됨 |
| 위즈코어 | 온프레미스 K8s 플랫폼 엔지니어(시니어) | 333230 | 「GPU 자원 배분 체계 수립 — 노드 풀 분리, MIG 프로파일, Kueue 큐·쿼터 정책... Vault HA 구성 및 운영... 인터넷·폐쇄망 이원 운영 — Harbor 복제, 패키지 미러」— 원래 「임베디드 개발자」로 잡혀 있었으나 실제 열린 req는 이 K8s 플랫폼 엔지니어였다 |
| 와탭랩스 | Kubernetes 에이전트 개발자 | 291531 | 「Kubernetes 클러스터 내 Node, Pod, Container 등 핵심 오브젝트의 메트릭 수집 파이프라인을 개발... NVIDIA GPU, MIG 등 AI 인프라 자원에 대한 모니터링 서비스를 개발」— ⚠ 이름의 "에이전트"는 LLM AI 에이전트가 아니라 모니터링 에이전트(Datadog류)다. 그래도 관측성 파이프라인 구현이라 시스템 축 |
| 마이클(마카롱팩토리) | Platform Engineering(5년 이상) | 333599 | 「표준 템플릿: 새 서비스를 시작할 때 쓰는 기본 골격을 제공... CI, 배포, 관측성, 시크릿 관리가 처음부터 붙어 있게 합니다... AI로 운영 자동화: 사람이 매번 처리하는 운영 판단과 반복 요청을 워크플로우와 AI로 옮기고」— 원래 노트의 "Data Engineering"이 아니라 이 Platform Engineering req가 시스템 축에 더 가깝다 |

### ⛔ 서비스 축 — 거른다

| 회사 | 직무 | 리멤버 id | 담당업무 인용 (판정 근거) |
|---|---|---|---|
| 스켈터랩스 | Forward Deployment Engineer(FDE/AI Engineer) | 332849 | 「고객사 비즈니스 요구사항 분석 및 LLM, RAG, NLP 기반 맞춤형 AI 솔루션 아키텍처 설계... 고객사 환경 현장 배포, QA, 안정화 지원」 |
| 인포시즈 | Forward Deployed Engineer(1-5년, 신설 주니어) | 322149 | 「고객사의 운영 과정과 문제를 분석하고 해결 방향을 설계... 단순 기능 구현이 아니라, 고객의 운영 방식과 의사결정 구조 자체를 개선합니다」 |
| 인핸스 | Forward Deployed Engineer(7년 이상) | 335974 | 「고객사에 깊이 임베디드되어 모호한 비즈니스 문제를 정의부터 솔루션 구현까지 주도하고... 컨설턴트형 소프트웨어 엔지니어」— ⚠ 이미 `rejected`(325034, 07년이상)와 같은 경력대의 별개 id. 재공고인지 신규 req인지는 확인 못 함 |
| 대동애그테크 | 사내 시스템 개발·운영 | 340213 | 「그룹웨어·인사·구매·영업 등 기업 솔루션 운영 및 기술지원... VoC 응대 및 사용자 대상 기술 가이드 제공」— AI/에이전트 축 자체가 없다. 원래 노트가 언급한 "영농 에이전트 및 스마트파밍 플랫폼 백엔드 개발자"는 지금 이 회사 검색 결과(총 2건)에 더 이상 없다 — 그 req는 닫힌 것으로 보인다. 마감 2026-09-27(D-9) |

### ⚠ 애매 — GLG가 고른다

| 회사 | 직무 | 리멤버 id | 시스템 쪽 문장 | 서비스 쪽 문장 |
|---|---|---|---|---|
| 파트리지시스템즈 | Forward Deployed Engineer | 331625 | 「데이터 수집, 전처리, 모델 적용, 검색, 리포트 생성까지 end-to-end 워크플로우를 구현합니다」 | 「고객 현장의 센서·영상·로그 데이터를 분석하고 해결해야 할 AI/Data 문제를 정의합니다... 고객 환경에 맞게 적용하고 PoC를 수행합니다」 |
| 마키나락스 | Forward Deployed Engineer - LLM | 313601 | 「Agent 연구개발을 통해 산업현장의 문제를 해결합니다... RAG, Fine tuning 등을 이용해 LLM 모델의 성능을 향상 시킵니다」 | 「제조·산업 데이터(설비 로그, 이미지, 문서 등)를 기반으로 LLM/VLM을 활용한 지능형 분석 및 자동화 시스템을 개발합니다」— 자사 제품(마키나락스 AI 솔루션) 축이라 순수 서비스는 아니지만, 산업현장 개별 적용이 핵심이라 순수 시스템도 아니다. 같은 회사에 LLM Quantization·Vision(Defect)·Lead(창원) 등 5개 이상 다른 req가 동시에 열려 있다(총 13건) |

### 확인 못 한 것

- **인핸스** — 실제 열린 req가 4개 이상이다(AI Backend Engineer, Product Engineer(AgentOS), AI Backend Engineer(Computer Use) 등). 이번엔 대표로 FDE(7년 이상, `335974`) 하나만 읽었다 — 나머지는 안 읽었다.
- **마키나락스** — 13개 req 중 대표로 「Forward Deployed Engineer - LLM」(`313601`) 하나만 읽었다. LLM Quantization·Vision(Defect Detection)·Lead(창원) 등은 안 읽었다.
- **위즈코어** — 원래 노트가 지목한 "[제조 AX 플랫폼] 임베디드 개발자" 타이틀은 이번 검색(총 46건) 카드 첫 화면에 안 보였다 — 그 정확한 req 대신 같은 회사의 「온프레미스 K8s 플랫폼 엔지니어」를 대표로 읽었다. 스크롤을 더 내리면 원래 지목한 req가 있을 수 있다.
- **대동애그테크** — 원래 노트의 "영농 에이전트 및 스마트파밍 플랫폼 백엔드 개발자"가 지금 검색 결과(총 2건)에 없다. 닫혔는지 제목이 바뀌었는지는 확인 못 했다.
