...
| 지원일 | — |
| 경로 | **원티드 자체 지원** (`wd/384224`) — `out_link: null` 실측 (2026-09-18) |
| 첨부 컷 | **Embedded Software Engineer** (`resume/build/KimJunghan_Resume_Embedded_Software_Engineer.pdf`) — MCU 실시간 소프트웨어·Fault Detection·Safety Logic 이 컷의 결정적 상태기계·양산 축과 대응한다. ⚠ PID 피드백 제어·모터 액추에이터는 증거 **없음** |
...
| 항목 | 값 | 영수증 |
|---|---|---|
| 공고 상태 | `active` — 살아 있다 | `GET /api/chaos/jobs/v1/384224/details` (2026-09-18 실측) |
| 마감일 | 없음(`due_time: null`) — 상시라는 보장이 아니라 **예고 없이 닫힌다**는 뜻 | 〃 |
| 경력 요건 | 4~10년 | 〃 `annual_from` / `annual_to` |
