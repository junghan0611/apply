...
| 항목 | 값 |
|---|---|
| 상태 | **draft** — 지원 URL·폼 확정. 남은 것은 **포트폴리오 조준부 3자리 교체 + 노트북 빌드** |
| 지원일 | — |
...
- [ ] **포트폴리오** — `dossier/` 산출물의 **조준부 3자리 교체 → 노트북 빌드 → `gs` 합본**.
      문안은 확정돼 있다 (§남은 하나). **NHN 제출본을 그대로 재사용할 수 없다** — 이유도 같은 절
...
## 남은 하나 — 포트폴리오 (⚠ 기존 제출본을 그대로 낼 수 없다)

이 폼에는 글을 쓸 자리가 없다. **포트폴리오가 유일하게 「왜 이 사람인가」를 말하는 문서다.**

### ⚠ 2026-07-29 서버 실측 — 재사용 가능성을 확인했고, 결과는 「조건부」다

NHN 제출본 2종(`../nhn--ax-transformation-ai-infra/submit/`)을 검사했다.

- ✅ **회사명은 없다.** `KimJunghan_AX_Portfolio.pdf`(12쪽 3.7M) · `KimJunghan_AX_Competency.pdf`
  (5쪽 324K) 본문 어디에도 NHN 이 찍혀 있지 않다. `dossier/` 소스의 NHN 언급은 전부
  `COMMENT`/`:noexport:` 블록이라 산출물로 나가지 않는다.
- ⛔ **그러나 「공고 대응」이 NHN 공고 번호에 묶여 있다.** 이게 그대로 못 내는 이유다.

| 위치 | 지금 박혀 있는 것 |
|---|---|
| `dossier/portfolio.org:45-51` 표 `공고 대응` 컬럼 | `주요 업무 ① 인프라 구축·서비스 설계·운영`, `주요 업무 ① 관리·운영`, `자격 — Agentic AI 활용 개발` … |
| `dossier/portfolio.org:53` | *"공고 두 번째 주요 업무인 AI 문서 작성·배포·교육에 대응합니다"* |
| `dossier/competency.org:189` 프롤로그 첫 문장 | *"'AX 전환 / AI 인프라(백엔드)' 공고의 주요 업무를 저는 두 겹의 역할로 읽었습니다"* |

**포트폴리오 첫 장이 다른 공고의 주요 업무 번호로 시작한다.** 서술형 칸이 없는 이 폼에서
첫 장은 곧 첫인상이다 — 조준이 어긋난 문서로 읽힌다.

> **구조적 발견**: `dossier/` 를 「회사 이름을 붙이지 않는 생성기」로 갈랐지만, **회사 중립과
> 공고 중립은 다르다.** 회사명은 지웠어도 조준부는 NHN 공고에 묶여 있다. 다음 회사마다
> 이 세 자리를 갈아 끼우게 된다 — 공고별 변수로 분리할 자리다(오늘 하지 않는다).

### ⭐ 채택안 — 표지 한 장을 앞에 붙인다 (org 소스는 건드리지 않는다)
...
- **org 소스 편집 0 · 재빌드 0 · 새로 쓰는 글 1장.**
- 표지가 맨 앞에 오므로 **첫인상이 한화비전 조준으로 바뀐다.** 뒤쪽 본문에 NHN 공고 번호
  매핑이 남아 있어도 「앞의 지도와 뒤의 본문」 관계로 읽힌다.
- 서술형 칸이 없는 폼에서 **표지가 사실상 자기소개서 자리**를 겸한다.

**✅ 표지 원고는 `cover-sheet.md` 로 확정했다 (2026-07-29).** 아래 ①②③ 이 그 구성이고,
파일에는 이미 반영돼 있다. **더 쓸 글은 없다** — 남은 것은 md → PDF 변환 하나뿐이다.
...
- [2026-07-29 오전, 오라클] **기존 제출본 재사용 검사 → 「조건부」.** 회사명은 없지만
  ...
  **기조 갱신(GLG)**: 기대값 낮춤 · 목적은 인력풀 소개 · 재래식 코딩테스트 요구 시 거절.
