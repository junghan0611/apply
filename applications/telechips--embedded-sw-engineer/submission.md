...
### ⚠ 충돌 — 포트폴리오 필수 vs 2026-07-30 판정

위 §「dossier 깊이 문서를 붙이지 않는다」는 **첨부가 선택 슬롯일 때의 판단**이었다.
이 폼은 포트폴리오를 **필수**로 받으므로 「안 붙인다」가 선택지가 아니다. 되살릴 때
**임베디드 축 조준 빌드**(`../deepx--linux-bsp-engineer/` 선례: 이력서 컷 +
`experience-detail.md` → `pdfunite`)를 먼저 뜨는 것이 답이다. AX Portfolio 12쪽을
그대로 내는 것은 이 자리의 답이 아니다.
