...
| 지원일 | — |
| 경로 | **원티드 자체 지원** (`wd/350980`) — `out_link: null` 실측 (2026-09-18) |
| 첨부 컷 | **Embedded Software Engineer** (`resume/build/KimJunghan_Resume_Embedded_Software_Engineer.pdf`) — 필수요건 3개가 전부 직접 공개 증거(임베디드 전주기·네이티브 개발·Edge AI 통합). 임베디드 축에서 가장 정면 |
...
| 항목 | 값 | 영수증 |
|---|---|---|
| 공고 상태 | `active` — 살아 있다 | `GET /api/chaos/jobs/v1/350980/details` (2026-09-18 실측) |
| 마감일 | 없음(`due_time: null`) — 상시라는 보장이 아니라 **예고 없이 닫힌다**는 뜻 | 〃 |
| 경력 요건 | 경력 무관 (`0` ~ 상한 미표기) | 〃 `annual_from` / `annual_to` |
