...
- [ ] **커버레터 — `cover-letter.md` 에 국문 본문을 썼다** (2026-07-29, 1,710자).
      제출 세트에는 `submit/cover-letter.txt` 로 깔려 있다.
      ⚠ **칸이 있는지는 아직 모른다** — 그리팅 `docsInfo` · `questionnairesInfo` 가 비어 있어
      기본 지원서만 받는 구성으로 보인다. **폼을 열어 확인한다.** 칸이 없으면 버리지 않고
      **1차 면접 「왜 우리인가」의 답 골격**으로 쓴다(코딩테스트 없는 전형이다).
