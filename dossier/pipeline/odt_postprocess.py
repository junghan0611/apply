...
def main():
    # 플래그를 위치 인자에서 분리한다. minirnd 제안서 양식은 캡션 번호를 '<표>' 라벨로
    # 바꾸지만(--strip-caption-numbers), NHN 제출본은 '표 1'/'그림 1' 연번을 유지해 본문에서
    # 참조한다. 그래서 캡션 번호 제거는 기본 OFF, opt-in.
    strip_caps = "--strip-caption-numbers" in sys.argv
    args = [a for a in sys.argv[1:] if not a.startswith("--")]
    if len(args) < 1:
        print(f"사용법: {sys.argv[0]} input.odt [output.odt] [--strip-caption-numbers]")
        sys.exit(1)

    input_odt = args[0]
    output_odt = args[1] if len(args) > 1 else input_odt
